...
import itertools
from make_test_vectors_Verilog import make_kernels_file, make_activations_file
class model_play():
    def __init__(self):
    ...
    for b,y,x,k in itertools.product(range(b_size), range(y_size), range(x_size), range(k_size)):
        ...
        for c,fy,fx in itertools.product(range(c_size), range(fy_size), range(fx_size)):
            if ((x+fx-fx_size//2) >= 0 and (x+fx-fx_size//2) < x_size and (y+fy-fy_size//2) >= 0 and (y+fy-fy_size//2) < y_size):
                interm_out += (features[b][y+fy-fy_size//2][c][x+fx-fx_size//2] * weights[c][fy][fx][k]).astype(np.int32)
...
if __name__ == "__main__":
    ...
    make_kernels_file(weights, 'weights.txt', 3, 3, 64, 64)
    make_activations_file(inputs, "activations.txt", 1, 56, 56, 64)
    ...
    make_activations_file(outputs_reference.astype(np.int32) , "outputs.txt", 1, 56, 56, 64)
